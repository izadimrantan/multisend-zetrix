...
import { generateBulkTransferBlobZTP1155, sign, submitTx } from "@/libs/zetrix";
...
export default function Send() {
  ...
  async function sendTransaction(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Generate Blob
    let { blob, blobHash } = await generateBulkTransferBlobZTP1155(walletAddress, ztpContract, recipientsInfo)
    if (blob) {
      // Get signature
      sign(blob)
        .then(async signData => {
          ...
          result && router.push(`/ztp1155/complete?tx=${txHash}`)
        })
        .catch(error => {
          setProcessing(false)
        })
            ...
            <div>
              ...
              <div className="overflow-x-auto py-6">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    <tr>
                      <td className="py-1">Token Contract (ZTP-1155)</td>
                      <td className="py-1 pl-2">{ztpContract}</td>
                    </tr>
                    <tr>
