...
import { processCSV } from "@/libs/utilities";
...
export default function Home() {
  // Global state
  const { walletAddress, recipientsInfo, setRecipientsInfo, setZTP20Contract } = useAppContext()

  // Router
  const router = useRouter()

  // Form input
  const ztp20Contract = useRef<HTMLInputElement>(null)
  ...
  async function handleZtp20ContractAddressChange(event: React.ChangeEvent<HTMLInputElement>) {
    const contractAddress = event.target.value
    if (contractAddress.trim() !== "") {
      const valid = await verifyZtpContractAddressExists(contractAddress)
      setIsValidContract(valid)
    }
  };
  ...
  function processFile(file: File) {
    ...
    reader.onload = async (event) => {
      ...
      try {
        const recipientsInfo = await processCSV(csvData)
  ...
  function formSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Update ZTP20 Contract
    setZTP20Contract(ztp20Contract.current!.value)

    // Redirect to Approve page
    router.push("/ztp20/approve")
  }
        ...
        <Card className="mt-6">
          <form onSubmit={formSubmit}>
            <div>
              ...
              <label htmlFor="contract">Token Contract (ZTP-20)</label>
              ...
              <div className="mt-2">
                <input
                  ...
                  ref={ztp20Contract}
                  onChange={handleZtp20ContractAddressChange}
                  required />
              </div>
              {!isValidContract && (
            ...
            <div className="mt-6">
              ...
              <p className="text-sm text-text_primary/80">List of recipient addresses in CSV format. You can download the CSV template{" "}
                <Link className="text-text_red/80 hover:text-text_red" href="/template.csv" download="template.csv">here</Link>.
              </p>
