...
import Progress from "@/components/progress_zetrix";
import TitlePrimary from "@/components/title_primary";
import { useAppContext } from "@/components/app";
import Link from "next/link";
import { useEffect, useState } from "react";
import { generateBulkTransferZetrixBlob, sign, submitTx } from "@/libs/zetrix";
...
export default function Send() {
  // Global state
  const { walletAddress, recipientsInfo } = useAppContext()
  ...
  async function sendTransaction(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Generate Blob
    let { blob, blobHash } = await generateBulkTransferZetrixBlob(walletAddress, recipientsInfo)
    if (blob) {
      // Get signature
      sign(blob)
        .then(async signData => {
          ...
          result && router.push(`/zetrix/complete?tx=${txHash}`)
        })
        .catch(error => {
          setProcessing(false)
        })
  ...
  return (
    <Container activeKey="zetrix">
      <div className="mx-auto max-w-xl">
        <div className="text-center">
          <TitlePrimary>Airdrop Tool</TitlePrimary>
          <p>Send ZETRIX tokens to multiple recipients at once</p>
        </div>
              ...
              <div className="overflow-x-auto py-6">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td className="py-1">Total Airdrop Amount (ZETRIX)</td>
                      <td className="py-1 pl-2">{totalAmount / 1000000}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
